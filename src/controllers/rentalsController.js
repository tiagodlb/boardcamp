  ...
  try {
    ...
    result.rows = result.rows.map((rental) => ({
      ...
      returnDate: rental.returnDate
        ? new Date(rental.returnDate).toLocaleDateString("en-CA")
        : null,
      originalPrice: rental.originalPrice,
      delayFee: rental.delayFee,
      customer: customerInfo.rows.find(
        (value) => rental.customerId === value.id
      ),
    ...
    if (customerId !== undefined && gameId !== undefined) {
      result.rows = result.rows.filter(
        (value) =>
          value.costumer.id === parseInt(customerId) &&
          value.game.id === parseInt(gameId)
      );
    ...
    if (customerId !== undefined && gameId === undefined) {
      result.rows = result.rows.filter(
        (value) => value.costumer.id === parseInt(customerId)
      );
      return res.send(result.rows);
    }
    ...
    if (gameId !== undefined && customerId === undefined) {
      result.rows = result.rows.filter(
        (value) => value.game.id === parseInt(gameId)
      );
      return res.send(result.rows);
    }
    res.send(result.rows);
  } catch (error) {
    return res.status(500).send("Error when you're trying to get rentals");
  }
}
...
export async function postRental(req, res) {
  ...
  try {
    const gettingGame = await connection.query(
    ...
    res.status(500).send("Error when you're trying to post rentals");
  }
}
...
export async function postRentalsReturn(req, res) {
  ...
  try {
    const rentalIdCheck = await connection.query(
    ...
    const devolutionInDays =
      new Date(rentalIdCheck.rows[0].rentDate).getTime() /
      (1000 * 60 * 60 * 24);
    const devolutionDate = new Date(
      (devolutionInDays + rentalIdCheck.rows[0].daysRented) *
        (1000 * 60 * 60 * 24)
    );
    const returnDate = new Date();

    const daysDiff = Math.floor(
      (returnDate.getTime() - devolutionDate.getTime()) / (1000 * 60 * 60 * 24)
    );
    ...
    await connection.query(
      ...
      [
        ...
        delayFee <= 0 ? 0 : delayFee,
        rentalId,
      ]
    );
    res.sendStatus(200);
  } catch (error) {
    res.status(500).send("Error when you're trying to post the rentals return");
  }
}
...
export async function deleteRentals(req, res) {
  ...
  try {
    ...
    res.status(500).send("Error when you're trying to delete a rental");
  }
}
